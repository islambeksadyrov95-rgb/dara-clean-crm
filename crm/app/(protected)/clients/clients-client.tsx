...
import { colorForSegment, segmentNames, computeSegment, DEFAULT_SEGMENT_RULES } from '@/lib/segments'
...
export function ClientsPageClient() {
  ...
  // Условия FilterBar восстанавливаются из URL (?f=) СИНХРОННО на первом рендере (как queue-client):
  // тогда queryKey клиента сразу совпадает с серверным SSR-префетчем даже при заданном ?f=, и
  // дегидрация срабатывает без мигания «Загрузка». Каждое изменение пишется в URL — фильтр можно
  // скинуть ссылкой, F5 не сбрасывает. window.location в инициализаторе безопасен: 'use client'.
  const [conditions, setConditions] = useState<FilterCondition[]>(() =>
    parseConditions(new URLSearchParams(window.location.search).get('f'))
  )
  ...
  // Список клиентов через TanStack Query: queryKey/queryFn — из общего модуля clients-query,
  ...
  // при пагинации/поиске (без скачка в «Загрузка»).
  const queryParams = { search: debouncedSearch, segment, page, conditions }
  const { data, isLoading, isError, error } = useQuery({
    queryKey: clientsListKey(queryParams),
