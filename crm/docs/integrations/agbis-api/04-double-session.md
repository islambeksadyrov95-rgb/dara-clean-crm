...
>
> ⚠ **Дока Агбиса местами врёт. Источник истины — `LIVE-VERIFIED-2026-06-18.md`** (снято с боевого API).
> Подтверждённые расхождения: выезд `tp` живьём = **1 забрать / 2 доставить** (не 0/1);
> `mp_status` в `Trips` = полный **0–6** (не 0/1/2); `LastChangeOrder` пуст; `order_plans` скрывает
> совершённые/отменённые выезды (статус выезда читать из `Trips`/`Trip`).
...
**Выезд `tp`:** ⚠ ЖИВОЙ API: **1 забрать · 2 доставить** (дока ошибочно говорит 0/1; в 302 боевых выездах нулей нет — enum совпадает с `TripOrder`)
**Выезд `kind_id`:** 1 по заказу · 2 по клиенту · 3 по складу
**Выезд `mp_status`:** ⚠ ЖИВОЙ API: полный **0 новый · 1 совершён · 2 отменён · 3 принял · 4 в пути · 5 приехал · 6 пересогласование** (дока для `Trips` ошибочно усекает до 0/1/2). `FullOrderInfo.order_plans.mp_status_id` показывает только 0/3/4 — совершённые/отменённые СКРЫТЫ
...
С `detail:1` → `trips[]`: `id`, `dt`, `hr`/`mn`, `to_hr`/`to_mn`, `tp` (⚠ ЖИВОЙ 1/2), `contr_id`, `contr_name`,
`contr_tel`, `contr_adr`, `dor_id`, `scl_name`, `sclad_plan_name`, `mp_status` (⚠ ЖИВОЙ 0–6), `car_scl_id`, `comment`,
