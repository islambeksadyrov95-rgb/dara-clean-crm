...
Промежуточный фикс: после полного слияния `orders`+`order_history` в одну таблицу кросс-табличный дедуп станет не нужен и будет снят (см. D-2026-06-16-orders-merge-deferred).
...
## D-2026-06-16-orders-merge-deferred [arch]
**Полное слияние `orders`+`order_history` в одну таблицу — ОТЛОЖЕНО.** Причина: `agbis-orders` — активный параллельный стрим (worktree `D:/Mind map/crm-agbis-orders`, ветка `agbis-orders` @ 6e32a2f, НЕ в main), который перестраивает запись заказов и весь UI заказов на ТОЙ ЖЕ `orders`: `TripOrder` (выезд/самовывоз) добавил на прод 10 колонок (`agbis_trip_id`, `delivery_address/date/type`, `intake_date`, `region_id`, `trip_window_from/to`, `fast_exec_id` — их нет ни в одной миграции main, поэтому `database.ts` в main расходится с продом), плюс переделка формы, `/orders`, `/orders/[id]`, order-outbox cron. Реструктурировать `orders` из main сейчас = гарантированная коллизия схемы+файлов на общем проде (ровно сценарий из AGENTS.md «streams collided on orders/types/database.ts»). `agbis-orders` сам уже двигает `orders` к Agbis-native модели — независимый restructure дублировал бы/конфликтовал. Срочности нет: двойной счёт полностью устранён дедупом (D-2026-06-16-recalc-dedup-cross-table).
План: unification делать ПОСЛЕ приземления `agbis-orders` в main, на одной целостной базе (или проектировать вместе с этим стримом). Карта 57 потребителей из сессии 16.06 сканировала только main — стрим `agbis-orders` в ней НЕ учтён, переиспользовать её для слияния нельзя без пересборки.
Rejected: параллельный restructure на отдельном worktree (коллизия с живым стримом); насильно слить agbis-orders в main под мой график (готовность стрима — бизнес-решение, не моё).
