...
- **Вывод для Фазы 2 (read/import) — три задачи, все бесплатные:**
  ...
  3. **Восстановление/сверка сумм — ENRICH, НЕ wipe (решение пользователя 16.06):** существующие строки `order_history` НЕ удаляем (телефоны/имена/даты/заказы уже верны). Тянем заказы Агбиса (`OrderByDateTimeForAll`, `kredit`/`dor_id`/`doc_date`/`user_name`/`Srvices[]`), матчим к существующим строкам по (`client_id` + `order_date`) one-to-one и ДОПОЛНЯЕМ недостающее: `amount`, `agbis_dor_id`, `agbis_doc_num`, `agbis_user_name`, `agbis_status_*` + строки в `order_history_items` (услуги). Несматченные заказы Агбиса (новые) → INSERT. Неоднозначный матч (несколько заказов клиента в один день) → жадно по порядку, остаток в карантин/лог. Идемпотентность вперёд — по `agbis_dor_id` (unique). Затем `recalc_client_aggregates`. Цель: вернуть ~93 млн ₸ корректных сумм + услуги + «кто оформил», ничего не теряя.
