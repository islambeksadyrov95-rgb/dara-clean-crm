...
## 🟢 РЕЗУЛЬТАТ КОНТРОЛИРУЕМОГО ТЕСТА — GO (2026-06-22)
Рецепт исполнен вживую на выбрасываемом заказе **dor 100354** (`binding/cancel_probe.py --dor 100354 --commit`):
- **Локально сразу после commit:** `DOCS_ORDER` status 7, KREDIT 0, DEBET 0. Строка услуги 100245410 обнулена, status 7, RETURNED 1. Возврат вставлен с id **1073 = 107‖3** (BI-триггер, ровно как предсказано по `GEN_CUR_DEP_ID=107` + `GEN_DOC_ORDER_SERV_RETURNS_ID`), без коллизии. Сработала защитная ветка: у заказа нашёлся **доп. услуга 100131337** с ненулевым `CHANGE_PRICE` → обнулён (⇒ фича ОБЯЗАНА обнулять допы). Очередь репликации: 4 строки `MST_META_CHANGES` (DOC_ORDER_SERVICES, DOC_ORDER_SERV_RETURNS, ADDON_ORDER_SERVICES, DOCS_ORDER), все `UNLOADED=0`.
- **Выгрузка вверх:** все 4 строки `UNLOADED 0→1` за единицы минут (DEP-дефолты строки возврата = 0, но репликация таблиц отмены depot-агностична — подтверждено).
- **Центр (make-or-break):** `OrderByDateTimeForAll` 22.06.2026 → заказ 100354 **status_id 7 + kredit 0 + услуга kredit 0**. Центр НЕ пересчитал и НЕ перетёр обнулённые деньги — принял raw-зануление как есть.
- **ВЫВОД: GO.** Сырая Firebird-отмена неоплаченного заказа работает end-to-end. Тулинг: `binding/cancel_probe.py` (запись по рецепту, транзакция+rollback), `binding/center_check.py` (двойной сигнал: локальный UNLOADED + REST центр).
- Заказ 100354 остаётся отменённым (это и есть желаемое конечное состояние тест-данных); возврат-строка 1073 — тест-данные.

## 🟢 СПАЙК ОТМЕНЫ ВЫЕЗДА — РЕШЕНО ТРИГГЕРОМ (2026-06-22)
Перед сборкой cancel-поллинга агента проверен механизм отмены выезда (read-only). Итог: **отдельный код агенту НЕ нужен.**
- `MOBILE_PLAN.MP_STATUS_ID` (0=Новый, 2=Отменён) — локальная колонка статуса выезда. REST `TripOrder mp_status=2` уже реализован в `lib/agbis/push-trip.ts` (`cancelArmTrip`, тесты есть) — но для нашей фичи он НЕ требуется.
- Триггер **`DOCS_ORDER_AIU_ZERO_TOVARS`** при `new.status_id=7`: обнуляет `doc_order_lines`+`doc_sclad_lines` (товары) И ставит `mobile_plan.mp_status_id=2` для каждого привязанного выезда — **но только если ВСЕ заказы плана стали 7** (`if not exists(... and dor.status_id <> 7)`). Shared-trip guard ВСТРОЕН.
- `if current_role='REPL_ROLE' then Exit` — триггер срабатывает на оригинальной записи агента (SYSDBA), пропускается на реплике центра (без двойной обработки).
- Доказано вживую: 25 отменённых заказов с выездом — у всех `mp_status=2`.
- **Следствие для T8:** агент делает ТОЛЬКО `UPDATE DOCS_ORDER status=7` (через `cancel_recipe`) + зеркало CRM; выезды отменяет триггер с корректным guard. Никакого REST `TripOrder`, никакого ручного guard. Up-репликацию trigger→MOBILE_PLAN проверить на e2e-тесте с реальным заказом-с-выездом.
