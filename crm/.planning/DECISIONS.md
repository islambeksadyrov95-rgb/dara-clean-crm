...
## D-2026-06-28-crm-scope
Владелец уточнил границы CRM (2026-06-28): CRM делает ТОЛЬКО — оформление заявок, приём/исходящие
звонки, рассылки, привязка выездов/доставок к заявкам. CRM НЕ делает: приём оплат (только ПОДТЯГИВАЕТ
из Агбиса read-only), обмер (курьер). Следствия:
1. **Оплаты read-only** — Фаза I больше НЕ PayForAll/Firebird-запись. Только тянуть статус оплаты
   (debet/dolg/paid) из Агбиса в CRM (read-sync). Гейт «2 настройки в центральной базе» СНЯТ —
   ничего не пишем в Агбис по деньгам.
2. **Локальный Firebird** (C:/Agbis/DB/ARM_7.FDB на админ-ПК) — исполнитель для привязки+отмены
   (binding/agent.py, запущен + autostart). Оплаты туда НЕ пишем (read-only из REST/order_history).
3. **Динамический Wazzup-канал** — номер БУДЕТ ДРУГОЙ. НЕ хардкодить номер/channelId в config.ts.
   Резолвить активный канал live из GET /v3/channels (любой подключённый номер). После QR нового
   номера всё работает само. На проде только WAZZUP_API_KEY (KEY_2 нет) → один аккаунт.
Rejected: PayForAll через локальный Firebird (как cancel) — владелец: оплаты вообще не наша задача,
только зеркалить из Агбиса.

## D-2026-06-29-broadcasts-variant-b
Рассылки WhatsApp переведены на durable серверную очередь (Variant B). Было: клиентский цикл в
broadcasts/page.tsx (useEffect+setInterval) — умирал при уходе со страницы, фон невозможен. Стало:
migration 20260629000001 — broadcast_campaigns (счётчики+статус, в realtime-публикации) +
broadcast_recipients (очередь, снимок ФИНАЛЬНОГО текста на момент запуска) + claim/settle RPC
(FOR UPDATE SKIP LOCKED). Процессор lib/broadcasts/drain.ts шлёт батч ~8/прогон с джиттером 3-6с
(анти-бан, ~1/мин на 10-мин cron) через УЖЕ динамический sendWhatsAppViaWazzup (живой active-канал
Wazzup → любой QR-подключённый номер). Дренаж пиггибэкнут на read-sync (/api/cron/agbis) + отдельный
/api/cron/broadcasts. startBroadcastCampaign возвращает сразу (фон); менеджер свободен. Сбой отправки
терминален (не ретраим — анти-бан). created_by = managerId для лога. Ручной режим broadcast_logs
сохранён.
Rejected: (1) Клиентский цикл — умирает при навигации. (2) Ретрай неудачных — риск бан-спама.
(3) Генерить AI-текст в cron — текст снимается в UI до запуска (менеджер контролирует/правит).
(4) Хардкод номера в config.ts для рассылки — отправка и так динамическая (D-2026-06-28-crm-scope).
