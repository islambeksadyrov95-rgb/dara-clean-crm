...
## 11. CRON-ПЛАНИРОВЩИК — заменить cron-job.org (работает плохо, нужна бесплатная надёжная замена)
**Нужно**: каждые ~10 мин дёргать `/api/cron/agbis?mode=increment&entity=all` (read-sync) + `/api/cron/agbis-orders` (дренаж outbox) с заголовком `Authorization: Bearer CRON_SECRET`. Vercel Hobby даёт только daily (1-2 джоба) → нужен внешний триггер. Сейчас это cron-job.org (один джоб + один токен = SPOF; владелец: «работает не очень»). Свежей сессии: оценить и внедрить надёжный БЕСПЛАТНЫЙ вариант + проверить что реально срабатывает (§10.4: `agbis_sync_state.last_run_at` vs РЕАЛЬНОЕ серверное время, не дата сессии).
**Бесплатные варианты (от лучшего к запасным):**
1. **Свой всегда-онлайн ПК** (тот же, где binding-агент) — Windows Task Scheduler + `curl` (или PowerShell `Invoke-WebRequest`) каждые N мин. Ноль третьих сторон, под контролем, надёжно. ИЛИ сам `binding/agent.py` пингует эти эндпоинты на таймере (у него уже есть Supabase-creds; добавить `CRON_SECRET` в `agent.config.json` + интервал). Минус: завязан на тот ПК — но агент и так там, а lease гарантирует один активный.
2. **Cloudflare Workers Cron Triggers** — free tier, расписание до 1 мин, Worker фетчит эндпоинт с секретом. Очень надёжно, отдельно от Vercel.
3. **Upstash QStash** — free ~500 сообщений/день (10 мин = 144/день, влезает), HTTP-расписание purpose-built + ретраи/дедлеттер.
4. **GitHub Actions schedule** (`on: schedule: cron: "*/10 * * * *"` → curl, секрет в GH Secrets) — бесплатно, НО тайминг неточный (опоздания 10-15+ мин под нагрузкой, мин. интервал ~5 мин). Как бэкап, не основной.
5. **Google Cloud Scheduler** — free 3 джоба, надёжный cron→HTTP, но нужен GCP-аккаунт/биллинг-setup.
6. Платно-но-чисто (если появится бюджет): **Vercel Pro** — нативный cron до 1 мин, убирает внешнюю зависимость СОВСЕМ.
**Рекомендация**: вариант 1 (свой ПК — он уже всегда онлайн ради агента) как основной + вариант 2/3 как облачный бэкап (два триггера = нет SPOF; дренаж идемпотентен через claim-очередь, повторный вызов безопасен). Лучше ДВА независимых триггера, чем один.
