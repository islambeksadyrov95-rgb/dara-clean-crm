;(function (global) {
  ...
  // Калиброваны по факту 2025: структура из раздела «2025 — Факт».
  const VARIABLE_SHARE = {
    production: 0.92, // ФОТ цех (сдельный) + поставщики + материалы = 100% переменные;
                      // коммунальные (~9% блока) = 40% переменные → взвешенный итог 0.92
    logistics:  0.80, // ГСМ и транспорт ТО (~71%) — 85% переменные;
                      // ФОТ логистика (~29%) — 70% переменный → итог 0.80
    marketing:  0.10, // 99% — платная реклама (Google/2ГИС/Yandex CPC).
                      // Это БЮДЖЕТНОЕ РЕШЕНИЕ, не авто-переменная — растёт по воле собственника
    sales:      0.07, // CRM/связь (~83%) — фиксированный SaaS;
                      // ФОТ продажи (~13%) — частично комиссии → итог ~0.07
    taxes:      0.00, // Налоги: реальная сумма сохраняется — растут только с инфляцией
    overhead:   0.10, // Интернет, прочие операционные — фиксированные подписки и сервисы
  }
